 ...
 */
public class WithdrawalSumAmoutListener implements UpdateListener {
    ...
    private void soutEventMessage(EventBean[] events){
        ...
        for (int i=0;i<events.length;i++)
        {
            ...
            System.out.println("by map  sum(amount):" + eventMap.get("sum(amount)"));
            System.out.println("by get method only sum(amount):" + events[i].get("sum(amount)"));
        }
    }
}
